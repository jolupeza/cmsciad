...
class Acl
{
    ...
    private $tables = array(
        ...
        'roles' => 'roles',
        'perms' => 'permissions',
        'user_perms' => 'users_permissions',
        'role_perms' => 'roles_permissions'
    );
    private $user_id;
    ...
    public function __construct($options = array())
    {
        ...
        if($this->user_id > 0) {
            ...
            $user_role = $this->CI->db->select('role_id')->get_where($this->tables['users'], array('id' => $this->user_id))->row();

            // setear id de role
            $this->user_role_id = isset($user_role->role_id) ? (int)$user_role->role_id : 0;
        }
    ...
    public function role_permissions_ids()
    {
        ...
        if($this->user_role_id > 0) {
            $perms = $this->CI->db
                    ...
                    ->get_where($this->tables['role_perms'], array('role_id' => $this->user_role_id))
                    ->result_array();
    ...
    public function role_permissions()
    {
        if($this->user_role_id > 0) {
            $permissions = $this->CI->db
                    ...
                    ->select(['r.permission_id', 'r.value', 'p.name', 'p.title'])
                    ->join($this->tables['perms'] . ' p', 'r.permission_id = p.id')
                    ->where(array('r.role_id' => $this->user_role_id))
                    ->get()->result();
